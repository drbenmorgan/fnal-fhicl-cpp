...
//==========================================================================
void
PrettifierAnnotated::before_action(key_t const& key, any_t const&, ParameterSet const* ps)
{
...
PrettifierAnnotated::enter_table(std::string const& key, boost::any const &)
{
  buffer_ << indent_()
          ...
          << print_annotated_info( curr_info_, cached_info_ )
          << nl();
  indent_.push();
}
...
void
PrettifierAnnotated::enter_sequence(std::string const& key,
                                    boost::any const& a)
{
  push_size_(a);
  buffer_ << indent_()
...
void
PrettifierAnnotated::exit_sequence(std::string const& key,
                                   boost::any const&)
{
  indent_.pop();
  buffer_ << indent_()
