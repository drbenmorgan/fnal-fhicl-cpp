...
namespace {
  ...
  inline bool allowed_info(std::string const& src_info)
  {
...
std::string detail::print_annotated_info(std::string const& curr_info,
                                         std::string const& cached_info)
{
  if ( !allowed_info(curr_info) ) return "";
  std::string const tag  = "  # ";
  std::string const printed_info = curr_info == cached_info ? "\"\"" : curr_info;
...
std::string detail::printed_suffix(std::string const& key, std::size_t const sz)
{
  std::string result;
...
std::string table::printed_prefix(std::string const& key)
{
...
std::string sequence::printed_prefix(std::string const& key)
{
  std::string result { opening_brace() };
  if ( !is_sequence_element(key) )
    result = key+": " + result;
...
std::string atom::printed_prefix(std::string const& key)
{
  return is_sequence_element(key) ? "" : key+": ";
}

std::string atom::value(boost::any const& a)
{
