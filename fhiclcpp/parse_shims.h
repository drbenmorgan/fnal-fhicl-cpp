...
namespace spirit {
template <>
struct use_terminal<::boost::spirit::qi::domain, shims::tag::catchall> : mpl::true_ {};
}
}

namespace shims {

inline bool maximally_munched_ass(char ch) {
  return !std::isgraph(ch) || ch == '#' || ch == '/' || ch == ',' || ch == ']' || ch == '}' || ch == '.' || ch == '[' ||
         ch == ':';
}

struct catchall_parser : ::boost::spirit::qi::primitive_parser<catchall_parser> {
  ...
  bool parse(Iterator& first, Iterator const& last, Context&, Skipper const& skipper, Attribute& attr) const {
    ...
    if (result.empty() || std::isdigit(result[0])) return false;
