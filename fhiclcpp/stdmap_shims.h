...
struct map {
  ...
  static_assert(std::is_same<typename mapmap_t::key_type, typename listmap_t::value_type::first_type>::value,
                "type mismatch for key_type");
  static_assert(std::is_same<typename mapmap_t::mapped_type, typename listmap_t::value_type::second_type>::value,
                "type mismatch for mapped_type");
  static_assert(std::is_same<typename mapmap_t::value_type, typename listmap_t::value_type>::value,
                "type mismatch for value_type");
  static_assert(std::is_same<typename mapmap_t::size_type, typename listmap_t::size_type>::value,
  ...
  struct iter : std::iterator<Category, TT, Distance, Pointer, Reference> {
    ...
    TT& operator*() { return isSnippetMode() ? *_iters.listmap_iter : *_iters.mapmap_iter; }

    TT* operator->() { return isSnippetMode() ? &*_iters.listmap_iter : &*_iters.mapmap_iter; }
    TT const* operator->() const { return isSnippetMode() ? &*_iters.listmap_iter : &*_iters.mapmap_iter; }

    TT& operator++() { return isSnippetMode() ? *(_iters.listmap_iter++) : *(_iters.mapmap_iter++); }

    bool operator==(iter other) const {
      return isSnippetMode() ? _iters.listmap_iter == other._iters.listmap_iter
                              : _iters.mapmap_iter == other._iters.mapmap_iter;
    }

    bool operator!=(iter other) const { return !operator==(other); }

    template <typename II>
    typename std::enable_if<std::is_same<typename mapmap_t::iterator, II>::value, II>::type get(II) {
  ...
  iterator begin() {
    return isSnippetMode() ? iterator{std::begin(_maps.listmap)} : iterator{std::begin(_maps.mapmap)};
  }

  const_iterator begin() const {
    maps_tuple& maps = *const_cast<maps_tuple*>(&_maps);

    return isSnippetMode() ? const_iterator{std::begin(maps.listmap)} : const_iterator{std::begin(maps.mapmap)};
  }

  const_iterator cbegin() const { return begin(); }

  iterator end() { return isSnippetMode() ? iterator{std::end(_maps.listmap)} : iterator{std::end(_maps.mapmap)}; }

  const_iterator end() const {
    maps_tuple& maps = *const_cast<maps_tuple*>(&_maps);

    return isSnippetMode() ? const_iterator{std::end(maps.listmap)} : const_iterator{std::end(maps.mapmap)};
  }
  ...
  T& at(Key const& key) {
    if (isSnippetMode()) {
      auto it = std::find_if(_maps.listmap.begin(), _maps.listmap.end(),
                             [&key](auto& element) { return element.first == key; });
      if (it == _maps.listmap.end()) throw std::out_of_range("Key <" + key + "> not found.");
      return it->second;
    } else {
  ...
  T const& at(Key const& key) const {
    if (isSnippetMode()) {
      auto it = std::find_if(_maps.listmap.cbegin(), _maps.listmap.cend(),
                             [&key](auto const& element) { return element.first == key; });
      if (it == _maps.listmap.cend()) throw std::out_of_range("Key <" + key + "> not found.");
      return it->second;
    } else {
      return _maps.mapmap.at(key);
    }
  ...
  iterator find(Key const& key) {
    if (isSnippetMode()) {
      return std::find_if(_maps.listmap.begin(), _maps.listmap.end(),
                          [&key](auto& element) { return element.first == key; });
    } else {
  ...
  const_iterator find(Key const& key) const {
    ...
    if (isSnippetMode()) {
      return std::find_if(maps.listmap.begin(), maps.listmap.end(),
                          [&key](auto const& element) { return element.first == key; });
    } else {
  ...
  bool empty() const { return isSnippetMode() ? _maps.listmap.empty() : _maps.mapmap.empty(); }

  size_type size() const { return isSnippetMode() ? _maps.listmap.size() : _maps.mapmap.size(); }
