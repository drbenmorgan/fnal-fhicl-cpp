...
namespace fhicl {
  ...
  public:
    ...
    auto const& operator()() const { return hls_; }
  };

  std::ostream& operator<<(std::ostream& os,  Vec const& v)
  {
    return os << "Half-lengths are: (" << v()[0] << ", " << v()[1] << ", " << v()[2] << ")";
  }
  ...
  struct CondConfig {
    ...
    bool flag_is_true() const { return flag(); }
    ...
    struct BoxParameters {
      TupleAs<Vec(int,int,int)> halfLengths { Name("halfLengths"), Vec{1,2,3} };
    ...
    Table<SphereParameters> sphereParams { Name("parameters"),
