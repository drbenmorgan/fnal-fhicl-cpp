...
bool
SearchAllowedConfiguration::supports_key(ParameterBase const& pb, std::string const& searched_for_key)
{
                                          ...
                                          std::string const& searched_for_key)
{
  ...
  if (!is_table(pt) && !is_sequence(pt)) {
    throw exception{error::other, "SearchAllowedConfiguration::form_absolute"}
    << "Attempt to search for '" << searched_for_key << "' as supported by a\n"
