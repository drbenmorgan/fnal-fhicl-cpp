...
namespace {
  ...
  struct RefS {
    ...
    RefS( int const i,
          int const sj, int const sk,
          int const tl, string const str, bool const flag ) :
      i_ (i),
      sj_(sj), sk_(sk),
      tl_(tl), str_(str), flag_(flag)
    {}
  ...
  {
    os << " Atom: " << s.atom()
       << " Sequence: [ " << s.sequence(0)  << ", " << s.sequence(1) << " ]"
       << " Tuple: [ "
       << s.tuple.get<0>() << ", "
       << s.tuple.get<1>() << ", "
